...
export const NodeInspector: React.FC<NodeInspectorProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-slate-800 bg-slate-950/95 p-6 shadow-2xl backdrop-blur-xl transition-all duration-300">
      ...
      <div className="flex items-start justify-between pb-4 border-b border-slate-800">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-white">{node.label}</h2>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto py-6 space-y-6">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          ...
          <a
            ...
            className="mt-2 flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm font-medium text-emerald-400 hover:border-emerald-500 hover:bg-slate-800/90 transition group"
          >
              ...
              {prereqNodes.map(({ node: pNode, isCompleted: pDone }) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={pDone ? 'text-slate-200 line-through' : 'text-slate-300 font-medium'}>
                      {pNode.label}
                    </span>
                  </div>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
            ...
            <div className="mt-3 space-y-2">
              {dependentNodes.map(dNode => (
                ...
                >
                  <span>{dNode.label}</span>
                  <span className="text-[10px] text-slate-500 font-semibold">{dNode.category}</span>
                </div>
              ))}
            </div>
          </div>
